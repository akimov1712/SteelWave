...
class TargetAdapter : ListAdapter<TargetModel, TargetViewHolder>(TargetDiffCallback()) {
    ...
    var onClickBtnMoreListener: ((TargetModel, View) -> Unit)? = null
    ...
    override fun onBindViewHolder(holder: TargetViewHolder, position: Int) {
        ...
        with(holder.binding) {
            ...
            val percent =
                ((item.collectedPrice.toFloat() / item.totalPrice.toFloat()) * 100).toInt()
            ...
            btnMore.btnMore.setOnClickListener{
                onClickBtnMoreListener?.invoke(item, it)
            }
        }
    }
}